...
<body class="font-poppins text-black">
    <section id="content" class="max-w-[800pxpx] w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-[120px]">
        ...
        <form method="POST" action="{{route('front.book.store', $packageWedding->slug)}}" class="flex flex-col gap-8">
          @csrf
          <div class="flex flex-col gap-3 px-4 ">
            <p class="font-semibold">wedding Date (Please book in advance)</p>
            <div class="flex items-center gap-[10px] bg-white p-[16px_24px] rounded-[37px]  transition-all duration-300">
              <input type="date" name="wedding_date" id="wedding_date" class="appearance-none outline-none w-full relative text-sm tracking-035 leading-[22px] [&::-webkit-calendar-picker-indicator]:absolute [&::-webkit-calendar-picker-indicator]:w-full [&::-webkit-calendar-picker-indicator]:h-full [&::-webkit-calendar-picker-indicator]:opacity-0">
          ...
          <div class="flex flex-col gap-3 px-4 ">
            ...
            <div class="bg-white p-4 rounded-[26px] flex items-center gap-3">
              ...
              <div class="flex flex-col gap-1">
                <p class="font-semibold text-sm tracking-035 leading-[22px]">{{$packageWedding->name}}</p>
                <div class="flex gap-1 items-center">
                  <div class="w-4 h-4">
                    <img src="{{asset('assets/icons/location-map.svg')}}" class="w-4 h-4" alt="icon">
                  </div>
                  <span class="text-darkGrey text-sm tracking-035 leading-[22px]">{{$packageWedding->location}}</span>
                </div>
              </div>
              <div class="flex flex-1 items-center justify-end gap-2">
                ...
                <input type="hidden" name="packageWeddingPrice" id="packageWeddingPrice" value="{{$packageWedding->price}}" />
          ...
          <div class="navigation-bar fixed bottom-0 z-50 max-w-[800px] w-full h-[85px] bg-black rounded-t-[25px] flex items-center justify-between px-6 shadow-[-6px_-8px_20px_0_#00000008]">
