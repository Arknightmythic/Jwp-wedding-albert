...
<body class="font-poppins text-black">
    <section id="content" class="max-w-[800pxpx] w-full mx-auto bg-[#F9F2EF] min-h-screen flex flex-col gap-8 pb-[120px]">
        ...
        <div id="image-details" class="px-4 flex justifty-center items-center flex-col gap-3">
          <div class="photod flex shrink-0 rounded-xl overflow-hidden">
            <img id="image-thumbnail" src="{{Storage::url($packageWedding->thumbnail)}}" class="w-full h-full object-cover object-center" alt="thumbnail">
          </div>
          <div class="grid grid-cols-4 gap-4 w-fit mx-auto">
            <a href="{{Storage::url($packageWedding->thumbnail)}}" class="thumbnail-option w-[74px] h-[74px] flex shrink-0 rounded-xl border-2 overflow-hidden mx-auto border-blue">
              <img src="{{Storage::url($packageWedding->thumbnail)}}" class="w-full h-full object-cover object-center" alt="thumbnail">
            </a>
        ...
        <div class="mx-4 flex flex-col gap-3 bg-white p-[16px_24px] rounded-[22px]">
          <h1 class="font-semibold">{{$packageWedding->name}}</h1>
          <div class="flex justify-between gap-2">
            <div class="flex items-center gap-2">
              ...
              <div class="flex flex-col gap-1">
                ...
                <p class="font-semibold text-sm tracking-035">{{$packageWedding->location}}</p>
              </div>
            </div>
            <div class="flex flex-col gap-1">
        ...
        <div class="mx-4 flex flex-col gap-3 bg-white p-[16px_24px] rounded-[22px]">
          ...
          <p id="readMore" class="text-sm leading-[22px] tracking-035 text-darkGrey">
            {{substr($packageWedding->about, 0 , 100)}}
        ...
        <div class="navigation-bar fixed bottom-0 z-50 max-w-[800px] w-full h-[85px] bg-white rounded-t-[25px] flex items-center justify-between px-6">
          <div class="flex flex-col justify-center gap-1">
            <p class="text-darkGrey text-sm tracking-035 leading-[22px]">Total Price</p>
            <p class="text-blue font-semibold text-lg leading-[26px] tracking-[0.6px]">Rp {{number_format($packageWedding->price,0,',','.')}}<span class="font-normal text-sx leading-[20px] tracking-035 text-darkGrey">/pack</span></p>
          </div>
          <a href="{{route('front.book', $packageWedding->slug)}}" class="p-[16px_24px] rounded-xl bg-blue w-fit text-white hover:bg-[#06C755] transition-all duration-300">Book Now</a>
        </div>
    </section>
